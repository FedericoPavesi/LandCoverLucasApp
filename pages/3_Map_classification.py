...
st.markdown('In previous section, we evaluated performances in a train, validation and test approach. This is very good for a first evaluation of the model but, as we would like to __assess the real-case performance of a land-cover classifier__, we cannot rely only on test accuracy and confusion matrix. Ideally, we would like to perform a comparison between a reliable ground truth classification mask on sentinel-2 images and our classification. Such information is unfortunately not available but we have an alternative: Lucas statistics. We know this dataset contains land cover areal statistics grouped by NUTS until level 2. What we can do is to create an image covering only a certain area (in this case we will use a region), then apply the classifier on the whole region to compute land cover statistics. Finally, we __compare results from the classifier with ground truth coming from Lucas statistics__. The intuition is again very simple: the __closest we get to Lucas regional statistics, the better the classifier in predicting land cover classes__.\n First step is the creation of reflectance map of the region of interest. Because this process is quite computational expensive, we shall choose a single region and focus on it. This limitation weakens the information of this evaluation procedure, through which we could have enlighten strengths and weaknesses of a classifier. Moreover, this exposes us to the risk of underestimating or overestimating the accuracy of the map. Suppose for example the case of a region with each land cover class almost perfectly balanced (for 8 classes, each one covers 12.5% of region surface), in this case we would find a random classifier to produce incredibly high performances both in terms of accuracy and computational speed. Anyway, this evaluation would be not correct as the prediction for each pixel is purely casual. This risk is of course reduced by the train, validation and test assessment, but still the lack of numerous samples (regions) jeopardizes the validity of any proposition based on these results. For this example, we chose __Italian region Lazio__. We again employ Earth Engine. We start by defining the region of interest as a multi-polygon representing ISTAT administrative boundaries of the region. We launch a query to __retrieve all the images covering this area in 2018 with cloud coverage lower than 40%__. We then __reduce the image collection to median reflectance of each pixel__ (method is the same as for the creation of the database explained in previous section). At this point, we use the shape of the multi-polygon to crop the image, this way we obtain a map in which only pixels belonging to Lazio territory would be saved while out-of-area pixels are excluded. Finally, we export the image to Google Drive  divided in lower size slices and we download locally each of them. Because we need to have squared slices, out-of-area points are recorded with each spectral band value to be equal to zero. At this point we __apply RF and MLP__ on all the slices and compute regional statistics on the predictions. __To apply 3x3 classifiers, we use a striding procedure in which for each single pixel it takes into account its 3x3 neighbourhood__. Moreover, for 3x3 algorithms it was necessary to add an artificial margin in order to avoid border issues. Specifically, the padding procedure uses the edge values of the image.')
...
st.markdown('Figures below provide an overview of the __comparison between Lucas statistics and statistics obtained by applying our classifiers__ (respectively, random forest 1x1, multi-layer perceptron 1x1, random forest 3x3 and multi-layer perceptron 3x3). Notice for __RF 3x3 we were only able to compute two trials as computational time was prohibitive__. What we can appreciate is all algorithms produce distributions relatively close to each other, with a Kullback-Leibler divergence from Lucas distribution of around 0.42. RF 3x3 seems the closest one with a score of around 0.41, while conversely MLP 3x3 seems most different scoring 0.46. Unfortunately, __even considering the closest prediction, we are far from producing a reliable soil classification__. All classifiers tend to largely overestimate shrubland, wetlands and water; while at the same time they considerably underestimate croplands and woodlands. Anyway, is __something we should expect__: as mentioned previously, __land cover classes are defined by something that goes beyond pure reflectance values__, they are complex structures of somehow interacting pixels. Sticking to previous example, a cropland could be defined as a specific structure of a network of pixels: if we see only one green pixel, it is hard to tell what it might belong to, looking instead at a parallelepiped shaped cluster of green pixels, we are prone to think it is a cropland, if we see a collection of adjacent structures of this kind (with eventually some interruption as a street for example) confidence our guess was correct is significantly increased. Moreover, we can spot two other sources of bias in this approach. First, as we already discussed, choosing pixel’s median reflectance especially over one year involves seasonality issues (like snow coverage , water level or croplands stages). Second, it is not simple to tell in which measure Lucas statistics are precise as they are computed using a survey approach (manually weighting recorded points) and we might assume they had to face issues similar to what we encountered (as for example seasonality for water levels).')
